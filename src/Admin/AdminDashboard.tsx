...
  const [filter, setFilter] = useState<"week" | "month" | "year">("month"); // Default to month
...
    // 2. Fetch Analytics (Charts) - REVISED LOGIC
    const fetchAnalytics = async () => {
      try {
        setLoadingAnalytics(true);
        // Get all paid transactions first
...
        
        if (filter === "year") {
          // YEAR: Jan - Dec fixed array
          const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
          formattedData = months.map(m => ({ label: m, revenue: 0 }));

          // Filter for current year only
          filteredTransactions = filteredTransactions.filter((t: any) => new Date(t.paid_at).getFullYear() === currentYear);
...
          // MONTH: Days 1 - 30/31 fixed array
...
          formattedData = Array.from({ length: daysInMonth }, (_, i) => ({ label: (i + 1).toString(), revenue: 0 }));
          
          filteredTransactions.forEach((t: any) => {
             const date = new Date(t.paid_at);
             const dayIndex = date.getDate() - 1;
             if(formattedData[dayIndex]) {
...
                    <IonCardTitle>Sales Analytics ({filter.charAt(0).toUpperCase() + filter.slice(1)})</IonCardTitle>
