...
const Staff_GenerateReports : React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                // ✨ BAG-ONG CONDITION: Revenue Report
    ...
    const generatePDF = () => {
        const doc = new jsPDF();
        doc.text(`${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report (Staff)`, 14, 15);
        ...
        doc.save(`${reportType}_report_staff.pdf`);
    };
    ...
    const generateExcel = () => {
        ...
        XLSX.writeFile(workbook, `${reportType}_report_staff.xlsx`);
    };
    ...
    // 🔥 CORE LOGIC: Verify OTP and Check for 'staff' Role (Unchanged)
                                        ...
                                        >
                                            ...
                                            {reportType !== "revenue" && (
                                                <>
                                                    ...
                                                    {(reportType === "bookings" || reportType === "transactions") && <td style={cellStyle}>{item.user_name || item.name || "-"}</td>} 
