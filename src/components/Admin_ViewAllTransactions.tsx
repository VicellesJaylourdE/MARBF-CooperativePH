...
import dayjs from "dayjs"; // Gamiton for formatting dates
...
interface Transaction {
    ...
    amount: number;
    // FIX: Include 'pending' status in the interface
    status: "unpaid" | "paid" | "cancelled" | "pending"; 
    payment_method: "cash" | "gcash" | string | null; 
...
const Admin_ViewAllTransactions: React.FC = () => {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            // Fetch transaction data with related bookings and users
    ...
    const filteredTransactions = useMemo(() => {
        return transactions.filter((t) => {
            ...
            // FIX: Ensure payment method comparison is case-insensitive for reliable filtering
            const paymentMethodLower = t.payment_method?.toLowerCase() || "";
            const paymentMatch =
                filterPayment === "all" || paymentMethodLower === filterPayment;
            ...
            <IonContent className="ion-padding">
                ...
                <div style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
                    ...
                    >
                        ...
                        {/* FIX: Gidugang ang 'pending' para maapil ang Cash transactions */}
                        ...
                        <IonGrid style={{ minWidth: "1200px" }}>
                            ...
                            {filteredTransactions.map((t, index) => (
                                ...
                                >
                                    ...
                                    {/* Display GCash Ref only if payment method is gcash (case-insensitive) */}
                                    <IonCol>
                                        {t.payment_method?.toLowerCase() === "gcash" ? t.gcash_ref_no || "N/A" : "-"}
                                    </IonCol>
                                    
                                    {/* Display Proof only if payment method is gcash and URL exists */}
                                    <IonCol>
                                        {t.payment_method?.toLowerCase() === "gcash" && t.proof_url ? (
                                            <img
                                                src={t.proof_url}
                                                alt="Proof"
                                                style={{
                                    ...
                                    <IonCol>
                                        {t.paid_at ? dayjs(t.paid_at).format("YYYY-MM-DD HH:mm") : "-"}
                                    </IonCol>
                                    <IonCol>{dayjs(t.created_at).format("YYYY-MM-DD HH:mm")}</IonCol>
                                    <IonCol>{dayjs(t.updated_at).format("YYYY-MM-DD HH:mm")}</IonCol>
                                </IonRow>
                            ))}
                        </IonGrid>
                    </div>
